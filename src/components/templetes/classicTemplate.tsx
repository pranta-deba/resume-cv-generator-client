  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-8 font-serif">
      ...
      <header className="text-center border-b-2 border-gray-800 pb-6 mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4 tracking-wide">{data.personalInfo.fullName}</h1>
          ...
          <div className="space-y-6">
            {data.experience.map((exp) => (
              <div key={exp.id}>
                ...
                {exp.description && <p className="text-gray-800 leading-relaxed text-justify">{exp.description}</p>}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-3">
            {data.skills.map((skillCategory) => (
              <div key={skillCategory.id}>
                <h3 className="font-bold text-gray-900 mb-1">{skillCategory.category}:</h3>
                <p className="text-gray-800">{skillCategory.items.join(", ")}</p>
              </div>
            ))}
          </div>
        </section>
            ...
            {data.projects.map((project) => (
              <div key={project.id}>
                <div className="flex justify-between items-start mb-2">
                  ...
                  {project.link && <span className="text-gray-600 text-sm">{project.link}</span>}
                </div>
                {project.technologies && (
                  <p className="text-gray-700 font-semibold mb-2">Technologies: {project.technologies}</p>
                )}
                {project.description && (
